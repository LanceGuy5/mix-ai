...
async function ConvertSongsToSpectrographs(songs) {
    for (const song of songs) {
        ...
        const audioFile = fs.createReadStream(path.join('songs', song), { start: 44 }) // first 44 bytes are the wav-header
        audioFile.pipe(sp)
        ...
        await new Promise((resolve, reject) => {
            ...
            sp.on('end', (err, data) => {
                ...
                // Draw spectrogram
                createImage(data, `${song.substring(0, song.length - 4)}_spectrogram`).then(() => {
                    sp.clear()
                    resolve()
                }).catch(reject)
            })
